...
import { generateFileListSync, hashContent } from "../utils.js";
...
describe("generateFileList", () => {
  test("returns all files when include is not specified", () => {
    ...
    const result = generateFileListSync({ rootDir });
  ...
  test("returns empty array when include is empty", () => {
    ...
    const result = generateFileListSync({ rootDir, include: [] });
  ...
  test("returns files matching exact filename", () => {
    ...
    const result1 = generateFileListSync({ rootDir, include: ["file1.txt"] });
    expect(result1).toEqual(["file1.txt"]);

    const result2 = generateFileListSync({ rootDir, include: ["file1.txt", "dir/file2.txt"] });
    expect(result2).toEqual(["dir/file2.txt", "file1.txt"]);

    const result3 = generateFileListSync({ rootDir, include: ["dir/subdir/file3.txt"] });
  ...
  test("returns files matching glob patterns", () => {
    ...
    const result1 = generateFileListSync({ rootDir, include: ["*.txt"] });
    expect(result1).toEqual(["file1.txt"]);

    const result2 = generateFileListSync({ rootDir, include: ["**/*.txt"] });
  ...
  test("returns includes directories & their contents", () => {
    ...
    const result1 = generateFileListSync({ rootDir, include: ["dir/**"] });
    expect(result1).toEqual(["dir/file2.txt", "dir/subdir/file3.txt"]);

    const result2 = generateFileListSync({ rootDir, include: ["dir"] });
    expect(result2).toEqual(["dir/file2.txt", "dir/subdir/file3.txt"]);

    const result3 = generateFileListSync({ rootDir, include: ["dir/"] });
  ...
  test("returns supports exclude", () => {
    ...
    const result1 = generateFileListSync({ rootDir, exclude: ["**/*.md"] });
  ...
  test("returns supports excludeFn", () => {
    ...
    const result1 = generateFileListSync({ rootDir, excludeFn: (path) => path.endsWith(".md") });
    expect(result1).toEqual(PATHS_TXT.sort());
  });
});
